...
int decode_to_float_vector(EncodedVectorGroup* encoded_vectors, int num_groups, float* result, int vector_length,
    ...
    // Apply inverse Hadamard transform if requested (at the top level)
    if (use_hadamard) {
        float *temp_output = (float *) malloc(vector_length * sizeof(float));
        apply_hadamard_transform(result, temp_output, vector_length);
        vector_divide(temp_output, result, vector_length, (float) vector_length);
    }
...
int decode_to_float_matrix(EncodedVectorGroup** encoded_matrices, int rows, int cols, float* result,
    bool use_hadamard, const float* row_norms) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        // Decode this row
        if (!decode_to_float_vector(encoded_matrices[i], num_groups, row_result, cols, use_hadamard, row_norm)) {
