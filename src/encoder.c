...
int encode_float_vector(const float* vector, int vector_length,  EncodedVectorGroup** encoded_vectors, bool use_hadamard, float* norm) {
    ...
    *encoded_vectors = (EncodedVectorGroup*)malloc(num_chunks * sizeof(EncodedVectorGroup));
    ...
    if (use_hadamard) {
        ...
        // Just normalize without Hadamard transform
        for (int i = 0; i < vector_length; i++) {
            transformed_vector[i] = vector[i] * normalization_factor;
        }
    }
...
int encode_float_matrix(const float* matrix, int rows, int cols,
                      EncodedVectorGroup*** encoded_matrices, bool use_hadamard, float* row_norms) {
    *encoded_matrices = (EncodedVectorGroup**)malloc(rows * sizeof(EncodedVectorGroup*));
    ...
    for (int i = 0; i < rows; i++) {
        ...
        if (!encode_float_vector(row, cols, &((*encoded_matrices)[i]), use_hadamard, row_norm_ptr)) {
            for (int j = 0; j < i; j++) {
