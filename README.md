...
1. **Reduced Memory Footprint**: Encodes high-dimensional vectors using only a few bits per dimension, achieving a compression ratio of 7.11:1 compared to standard floating-point representation.
...
- Vector norms are preserved through normalization during encoding)
...
```c
/**
 * Encode a vector using nested-lattice quantization
 *
 ...
 * @param use_hadamard Whether to apply inverse Hadamard transform during decoding
...
int decode_to_float_vector(EncodedVectorGroup* encoded_vectors, int num_groups, 
                          float* result, int vector_length, bool use_hadamard, 
                          float norm);
...
int decode_to_float_matrix(EncodedVectorGroup** encoded_matrices, int rows, 
                          int cols, float* result, bool use_hadamard, 
 ...
 * @param use_hadamard Whether Hadamard transform was used during encoding
...
int multiply_encoded_matrices(
    ...
    float* result, int8_t* lut, bool use_hadamard,
    const float* row_norms_a, const float* row_norms_b);
```
...
- `hadamard_flag`: Use Hadamard transform (0=disabled, 1=enabled, default: 1)
